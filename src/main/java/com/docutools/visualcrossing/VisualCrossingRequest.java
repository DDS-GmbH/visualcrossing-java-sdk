...
public record VisualCrossingRequest(String location, String timeframe, Map<String, String> params) {

    URI toURI(final String protocol, final String host, final String basePath, final String apiKey) {
        String path = String.format("%s/%s", basePath, encodeValue(location));
        path += buildTimeframePathPart();
        try {
            return new URI(protocol, host, path, buildQueryString(apiKey), "");
        } catch (URISyntaxException e) {
            throw new VisualCrossingException(
                    String.format("Could not create URI for API server: protocol=%s, host=%s, path=%s",
